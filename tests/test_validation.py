...
def test_calculate_rainfall_metrics_with_thresholds():
    ...
    metrics = plg.validation.calculate_rainfall_metrics(ref_array, est_array, ref_thresh=0.1, est_thresh=0.1)
    
    assert metrics["reference_rainfall_threshold"] == 0.1
    assert metrics["estimates_rainfall_threshold"] == 0.1
    assert np.isclose(metrics["pearson_correlation_coefficient"], -0.385, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["coefficient_of_variation"], 1.759, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["root_mean_square_error"], 0.803, atol=0.001, equal_nan=True)
    ...
    assert np.isclose(metrics["reference_mean_rainfall"], 0.456, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["estimated_mean_rainfall"], 0.456, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["reference_rainfall_sum"], 4.109, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["estimated_rainfall_sum"], 4.200, atol=0.001, equal_nan=True)   
    ...
    assert np.isclose(metrics["false_wet_rainfall_rate"], 1.0, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["missed_wet_rainfall_rate"], 1.0, atol=0.001, equal_nan=True)
...
def test_calculate_rainfall_metrics_with_nans():
    ...
    assert np.isclose(metrics["pearson_correlation_coefficient"], 0.1186, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["coefficient_of_variation"], 2.2739, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["root_mean_square_error"], 0.6123, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["mean_absolute_error"], 0.3987, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["percent_bias"], 46.9194, atol=0.001, equal_nan=True)
    
    assert np.isclose(metrics["reference_mean_rainfall"], 0.263, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["estimated_mean_rainfall"], 0.387, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["reference_rainfall_sum"], 2.11, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["estimated_rainfall_sum"], 3.10, atol=0.001, equal_nan=True)   
    ...
    assert np.isclose(metrics["false_wet_rainfall_rate"], 0.55, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["missed_wet_rainfall_rate"], 0.55, atol=0.001, equal_nan=True)
    assert metrics["N_all"] == 12
...
def test_calculate_rainfall_metrics_with_zeros():
    ...
    assert metrics["reference_rainfall_threshold"] == 0.0
    assert metrics["estimates_rainfall_threshold"] == 0.0
    assert np.isclose(metrics["pearson_correlation_coefficient"], np.nan, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["coefficient_of_variation"], np.inf, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["root_mean_square_error"], 0.578, atol=0.001, equal_nan=True)
    ...
    assert np.isclose(metrics["reference_mean_rainfall"], 0.0, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["estimated_mean_rainfall"], 0.350, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["reference_rainfall_sum"], 0.0, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["estimated_rainfall_sum"], 4.200, atol=0.001, equal_nan=True)   
    
    assert np.isclose(metrics["false_wet_ratio"], 0.5, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["missed_wet_ratio"], np.nan, atol=0.001, equal_nan=True)
    assert np.isclose(metrics["false_wet_rainfall_rate"], 0.700, atol=0.001, equal_nan=True)
...
def test_calculate_wet_dry_metrics_with_nans():
    ref_array = np.array([True, True, False, False, True, True, False, False, True, np.nan, np.nan, False])
    est_array = np.array([False, True, True, False, False, True, True, False, np.nan, np.nan, np.nan, False])
...
def test_calculate_wet_dry_metrics_replaced_with_zeros():
    ref_array = np.array([True, True, False, False, True, True, False, False, True, True, False, False])
    est_array = np.array([False, False, False, False, False, False, False, False, False, False, False, False])
